 ...
 */
package org.springframework.social.skplanetx.sample.user;
...
public final class UserInterceptor extends HandlerInterceptorAdapter {
    ...
    private boolean notRequestForLogin(HttpServletRequest request) {
        return request.getServletPath().equals("/signin") || request.getServletPath().equals("/signout");
    }
	...
	private void handleSignOut(HttpServletRequest request, HttpServletResponse response) {
		if (SecurityContext.userSignedIn() && request.getServletPath().startsWith("/signout")) {
			connectionRepository.createConnectionRepository(SecurityContext.getCurrentUser().getId()).removeConnections("skplanetx");
			userCookieGenerator.removeCookie(response);
			SecurityContext.remove();			
		}
	}
	...
	private boolean userNotFound(String userId) {
        return connectionRepository.createConnectionRepository(userId).findPrimaryConnection(Skplanetx.class) != null;
	}
